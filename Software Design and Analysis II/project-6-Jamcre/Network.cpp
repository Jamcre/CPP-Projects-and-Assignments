...
/*
 ...
 @param            :   A POINTER to an integer counter that gets incremented every time a COMPARISON between two Nodes occurs
...
LinkedList<Post *> Network<ItemType>::bSortByUsername(Comparator compare, int *swap_count)
{
    // create a deep copy of Network's LinkedList 'feed_'
    LinkedList<Post *> feed_copy(feed_);
    if (feed_copy.getSize() > 1)
    {
        // use LinkedList bubblesort function to sort copy of 'feed_'
        feed_copy.bubbleSort(compare, swap_count);
    }
...
template <typename ItemType>
void Network<ItemType>::printUsernames()
{
    Node<Post *> *header = feed_.getHeadPtr();

    while (header != nullptr)
    {
        std::cout << header->getItem()->getUsername() << std::endl;
        header = header->getNext();
    }
}
